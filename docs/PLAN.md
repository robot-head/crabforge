...
- **SQL surface constraints the forge schema must respect**: no JSONB (store JSON as `text`), no foreign keys (app-level integrity), no `INSERT … ON CONFLICT` (single-writer projector does read-then-write inside BEGIN/COMMIT), no LISTEN/NOTIFY (poll or gate on projector applied-offset), no savepoints, no schemas (all tables in `public`), no window functions, no arrays, CHECK rejected, indexes = single-column equality only, nested-loop joins. Types available: bool/int4/int8/text/float8/numeric/date/time/timestamp(tz)/interval/bytea.
...
- **Verified current as of 2026-07-28** (clone = origin/main HEAD `22b0f46`): grep confirms 0 `jsonb` hits in pgtypes/pgexec/pgparser/pgcatalog, 0 `ON CONFLICT` in the parser, Datum = 13 variants, no LISTEN/NOTIFY. Docs are not stale.
- **Co-dev stance on PG compatibility (user direction)**: crabforge writes standard Postgres SQL; where today's gres lacks a feature, the workaround is isolated in the storage layer and tagged `TODO(gres:<feature>)`; crabforge maintains `docs/gres-gaps.md` ranking missing SQL features by pain (ON CONFLICT, JSONB, LISTEN/NOTIFY, FKs, savepoints, schemas, …) as upstream crabka work items, and contributes forge-shaped statements to gres's conformance corpus. Workarounds get deleted as gres catches up.
...
6. **gres gaps program**: `docs/gres-gaps.md` (ON CONFLICT > JSONB > composite/range indexes > FKs > LISTEN/NOTIFY > savepoints > transactional-DDL answer) + forge-shaped conformance-corpus statements. 7. **gres G3 checkpoints** advocacy (cold start = full WAL replay is the biggest operability dependency).
